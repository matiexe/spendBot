...
def inicializar_bd():
    ...
    # Tabla de gastos (gastos + ingresos)
    cursor.execute('''
...
def registrar_transaccion(id_usuario, tipo, monto, categoria, descripcion='', 
                         detectado_por_ia=False, confianza_ia=None, cuenta='-', origen='Telegram'):
    """
    ...
    cursor.execute(
        '''INSERT INTO gastos 
           (id_usuario, tipo, monto, categoria_id, descripcion, 
            detectado_por_ia, confianza_ia, fecha, cuenta, origen) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
        (id_usuario, tipo.upper(), monto, categoria_id, descripcion,
         1 if detectado_por_ia else 0, confianza_ia, datetime.now(), cuenta, origen)
    )
